...
Builder.prototype.buildTypeFunction = function () {
    ...
    body +=
        ...
        '\tvar PlainMessage = arguments.callee.require(\'mtproto\').PlainMessage;\n' +
    ...
    body +=
        '\tconn.connect(function (ex) {\n' +
        '\t\tif(ex) {\n' +
        '\t\t\tthis.logger.error(\'Unable to connect: %s \', ex);\n' +
        '\t\t\tif(callback) callback(ex);\n' +
        '\t\t\treturn;\n' +
        '\t\t}\n' +
        '\t\tconn.write(reqMsg.serialize(), function (ex) {\n' +
        '\t\t\tif(ex) {\n' +
        '\t\t\t\tthis.logger.error(\'Unable to write: %s \', ex);\n' +
        '\t\t\t\tif(callback) callback(ex);\n' +
        '\t\t\t\treturn;\n' +
        '\t\t\t}\n' +
        '\t\t\tconn.read(function (ex, response) {\n' +
        '\t\t\t\tif(ex) {\n' +
        '\t\t\t\t\tthis.logger.error(\'Unable to read: %s \', ex);\n' +
        '\t\t\t\t\tif(callback) callback(ex);\n' +
        '\t\t\t\t\treturn;\n' +
        '\t\t\t\t}\n' +
        ...
        '\t\t\t\tif(callback) callback(null, resObj.deserialize());\n' +
        '\t\t\t});\n' +
        '\t\t});\n' +
        '\t});';
