...
Builder.prototype.buildTypeFunction = function () {
    ...
    body +=
        ...
        '\tvar type_language = self.require(\'type_language\');\n' +
    ...
    body +=
        ...
        '\t\t\t\tvar row = 1;\n' +
        '\t\t\t\ttry {\n' +
        '\t\t\t\t\tvar resMsg = new PlainMessage({buffer: response}).deserialize();row++;\n' +
        '\t\t\t\t\tresMsg = resMsg.getMessage();row++;\n' +
        '\t\t\t\t\tvar Type = type_language.Builder.retrieveType(resMsg);row++;\n' +
        '\t\t\t\t\tvar resObj = new Type({buffer: resMsg});row++;\n' +
        '\t\t\t\t\tresObj.deserialize();row++;\n' +
        '\t\t\t\t\tif(callback) callback(null, resObj);row++;\n' +
        '\t\t\t\t} catch(ex4) {\n' +
        '\t\t\t\t\tself.logger.error(\'Unable to deserialize response due to %s at row num %s \', ex4, row);\n' +
