...
// Import dependencies
var debug = require('debug')('telegram.link:mtproto');
var util = require('util');
var AbstractObject = require("./type_language").AbstractObject;
var Builder = require("./type_language").Builder;

// Declare the module to be exported
var mtproto = {};

// Define the `class` list to build in module `mtproto`
mtproto._classes = ['ResPQ', 'P_Q_inner_data'];

// Define the `static-method` list to build in module `mtproto`
mtproto._methods = ['req_pq'];
...
Builder.buildTypes(api.methods, mtproto._methods, mtproto, true);

// first req_pq method implementation (hand made):
//  {"id": "1615239032", "method": "req_pq", "params": [
//     {"name": "nonce", "type": "int128"}
//  ], "type": "ResPQ"},
mtproto.req_pq = function (options) {
    var conn = options.conn;
    if (!conn) {
        debug('The \'conn\' option is missing, connection is mandatory');
        return;
    }
    var callback = options.callback;
    var errorback = options.errorback;
    var reqPQ = new mtproto._req_pq(options);
    var reqMsg = new mtproto.PlainMessage({message: reqPQ.serialize()});
    conn.connect(function () {
        conn.write(reqMsg.serialize(), function () {
            conn.read(function (response) {
                ...
                var resPQ = new mtproto.ResPQ({buffer: resMsg});
                callback(resPQ.deserialize());
            }, errorback);
        }, errorback);
    });
};
...
mtproto.PlainMessage = function (options) {
    ...
    if (this._message) {
        ...
        this._authKeyId = 0;
        this._messageId = (new Date().getTime() / 1000) << 32;
        this._messageLength = this._message.length;
    }
};

util.inherits(mtproto.PlainMessage, AbstractObject);
