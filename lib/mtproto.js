...
(function (api, classes) {
    for (var i = 0; i < api.constructors.length; i++) {
        var klass = api.constructors[i];
        if (classes.lastIndexOf(klass.type) >= 0) {
            mtproto[klass.type] = Builder.build(klass);
        }
    }
})(api, mtproto._classes);
