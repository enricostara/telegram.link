                ...
                callback: function (ex, config) {
                    if (ex) {
                        ...
                        for (var i = 0; i < dcList.length; i++) {
                            var dc = dcList[i];
                            dcs['DC_' + dc.id] = {host: dc.ip_address, port: dc.port, toPrintable: tl.utility.toPrintable};
                        }
...
Client.prototype.signIn = function(phoneNumber, phoneCodeHash, phoneCode, callback){
    if(callback){
...
Client.prototype.getContacts = function(contactHash , callback){
    if(callback){
        this.once('getContacts',callback)
    }
    var props={
        hash: contactHash
    };
    if(this.isReady(true)){
        try{
            api.service.contacts.getContacts({
                props: props ,
                channel: this._channel,
                callback: createEventEmitterCallback('getContacts').bind(this)
            });
        }catch(err){
            this.emit('error',err);
        }
