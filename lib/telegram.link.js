...
//      - **authKeyBuffer** && **authKeyPassword** _(optional)_: the authentication key buffer to decrypt with the password
...
function Client(app, dataCenter) {
    ...
    if ('TCP' === app.connectionType) {
        this._connection = new net.TcpConnection(dataCenter);
    } else {
        this._connection = new net.HttpConnection(dataCenter);
    }

    app.authKey = app.authKeyBuffer && app.authKeyPassword ?
        auth.AuthKey.decryptAuthKey(app.authKeyBuffer, app.authKeyPassword) :
        app.authKey;
...
Client.prototype.createAuthKey = function (callback) {
    ...
    if (this._connection && this._connection.isConnected()) {
        auth.createAuthKey(
            createEventEmitterCallback('authKeyCreate').bind(this),
...
// ***
// client.**exportEncryptedAuthKey(password)**

// Exports an encrypted buffer of the internal authKey using the password.

// The code:
Client.prototype.exportEncryptedAuthKey = function (password) {
    return this._channel ? auth.AuthKey.encryptAuthKey(this._channel._context.authKey, password) : null;
}
    ...
    if (this.isReady(true)) {
        try {
            api.service.auth.sendCode({
                ...
                callback: createEventEmitterCallback('sendCode').bind(this)
            });
        } catch (err) {
            this.emit('error', err);
        }
...
// ***
// client.**signIn(phoneNumber, [callback])**

// Signs in a user with a validated phone number

// The code:
Client.prototype.signIn = function (phoneNumber, codeHash, code, callback) {
    if (callback) {
        this.once('signIn', callback);
    }
    var props = {
        phone_number: phoneNumber,
    ...
    if (this.isReady(true)) {
        try {
            api.service.auth.signIn({
                props: props,
                channel: this._channel,
                callback: createEventEmitterCallback('signIn').bind(this)
            });
        } catch (err) {
            this.emit('error', err);
        }
...
Client.prototype.checkPhone = function (phoneNumber, callback) {
    if (callback) {
        this.once('checkPhone', callback)
    }
    var props = {
        phone_number: phoneNumber
    };
    if (this.isReady(true)) {
        try {
            api.service.auth.checkPhone({
                props: props,
                channel: this._channel,
                callback: createEventEmitterCallback('checkPhone').bind(this)
            });
        } catch (err) {
            this.emit('error', err);
        }
...
Client.prototype.signUp = function (phoneNumber, phoneCodeHash, phoneCode, firstName, lastName, callback) {
    if (callback) {
        this.once('checkPhone', callback)
    }
    var props = {
    ...
    if (this.isReady(true)) {
        try {
            api.service.auth.signUp({
                props: props,
                channel: this._channel,
                callback: createEventEmitterCallback('signUp').bind(this)
            });
        } catch (err) {
            this.emit('error', err);
        }
...
Client.prototype.end = function (callback) {
    ...
    this._connection.close(createEventEmitterCallback('end').bind(this));
};
...
// The code:
function connect() {
    this._connection.connect(createEventEmitterCallback('connect').bind(this));
}
