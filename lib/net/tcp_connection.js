...
function TcpConnection(options) {
    ...
    if (logger.isDebugEnabled()) logger.debug('created with ' + this._config);
}

TcpConnection.prototype.connect = function (callback, errorback) {
    if (logger.isDebugEnabled()) logger.debug('connecting to ' + this._config);
    ...
    var socket = net.connect(this.options, function () {
        if (logger.isDebugEnabled()) logger.debug('connected to ' + this._config);

        var abridgedFlag = new Buffer(1);
        abridgedFlag.writeUInt8(0xef, 0);
        if (logger.isDebugEnabled()) logger.debug('sending abridgedFlag to ' + this._config);
        socket.write(abridgedFlag, 'UTF8', function () {
            if (logger.isDebugEnabled()) logger.debug('abridgedFlag sent to ' + this._config);
            if (callback) {
                callback();
            }
        }.bind(this));
    ...
    if (errorback) {
        socket.on('error', function (e) {
            logger.error('Error %s connecting to %s', e.code, this._config);
...
TcpConnection.prototype.write = function (data, callback, errorback) {
    ...
    if (logger.isDebugEnabled()) logger.debug('writing %s bytes to %s', request.length, this._config);
    socket.write(request, 'UTF8', function () {
        if (logger.isDebugEnabled()) logger.debug('wrote %s bytes to %s', request.length, this._config);
        if (callback) {
            callback();
        }
    }.bind(this));
    ...
    if (errorback) {
        socket.on('error', function (e) {
            logger.error('Error %s writing %s bytes to %s', e.code, request.length, this._config);
            errorback(e);
        }.bind(this));
    }
};
...
TcpConnection.prototype.read = function (callback, errorback) {
    if (logger.isDebugEnabled()) logger.debug('reading from %s', this._config);
    var socket = this._socket;
    ...
    socket.on('data', function (data) {
        ...
        if (logger.isDebugEnabled()) logger.debug('read %s bytes from %s', payload.length, this._config);
    ...
    if (errorback) {
        socket.on('error', function (e) {
            logger.error('Error %s reading from %s', e.code, this._config);
            errorback(e);
        }.bind(this));
    }
};
...
TcpConnection.prototype.close = function (callback, errorback) {
    var socket = this._socket;
    if (socket) {
        if (logger.isDebugEnabled()) logger.debug('disconnecting from ' + this._config);

        socket.on('end', function () {
            if (logger.isDebugEnabled()) logger.debug('disconnected from ' + this._config);
            if (callback) {
                callback();
            }
        }.bind(this));
        if (errorback) {
            socket.on('error', function (e) {
                logger.error('Error %s disconnecting from %s', e.code, this._config);
                errorback(e);
            }.bind(this));
        }
        socket.end();
