...
TcpConnection.prototype.read = function (callback, errorback) {
    var self = this;
    if (logger.isDebugEnabled()) logger.debug('reading from %s, then callback ', self._config, callback);
    var socket = this._socket;
    if (!socket) {
        errorback({code: 'ENOTCONNECTED', msg: 'not yet connected'});
        return;
    ...
    var onData = function (data) {
        if (logger.isDebugEnabled()) logger.debug('removing %s and callback %s from onData event', arguments.callee, callback);
